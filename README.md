...
This project implements a *Domain-Specific Language (DSL)* for defining and managing game state in a Civilization-like 
game. The DSL allows users to define buildings, units, and cities with various attributes and relationships.

The project is built using *Langium*, a language development framework that simplifies the creation of DSLs and *Rust*
using `ratatui` for building terminal user interfaces.
...
To use the DSL in *Visual Studio Code*, you can install the provided extension using the
[`vscode-CLIvilization-0.0.1.vsix`](vscode-CLIvilization-0.0.1.vsix) package (right-click and "Install extension VSIX").
...
You first need to [Generate the AST](#generating-the-ast). The AST is used by the Rust compiler to generate a compatible
...
```bash
cd CLIvilization
node packages/cli/bin/cli.js generate <src>.civ <out>
```
