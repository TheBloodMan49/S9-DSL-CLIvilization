...
impl GameState {
    ...
    pub fn start_construction(
        ...
        let Some(bdef) = self.buildings.iter().find(|b| b.name == building_name) else {
    ...
    pub fn start_recruitment(&mut self, civ_index: usize, unit_name: &str) -> Result<(), String> {
        let Some(udef) = self.units.iter().find(|u| u.name == unit_name) else {
    ...
    pub fn cycle_zoom(&mut self) {
        ...
        match (old_zoom, self.zoom_level) {
            ...
            _ => {
                unreachable!("Invalid zoom transition");
            }
        }
    ...
    pub fn calculate_city_power(&self, civ_index: usize) -> i32 {
        ...
        for unit in &civ.city.units.units {
            ...
            power += unit.nb_units.cast_signed()
                * self
                    .units
                    .iter()
                    .find(|u| &u.name == id)
